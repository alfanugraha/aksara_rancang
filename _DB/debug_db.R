...
LDMProp_his<-readRDS(paste0(datapath,"LDMProp")) #beda sama mas alfa
...
scenario1EnergySE1 <- read.table("D:/My_Development/RProjects/lcd-scenario/_DB/18_persentase_bahan_bakar_sken1_tahap1.csv", header = F, sep = ",")
for(t in 2017:2026){
  scenarioConsumption <- scenario2CombSeriesOfImpactEnergy[[paste0("y", t)]][["consumption"]]
  scenario2CombSeriesOfImpactEnergy[[paste0("y", t)]][["consumption"]][4:ncol(scenarioConsumption)] <-  scenarioConsumption[4:ncol(scenarioConsumption)] * scenario1EnergySE1
}

scenario1EnergySE2 <- read.table("D:/My_Development/RProjects/lcd-scenario/_DB/18_persentase_bahan_bakar_sken1_tahap2.csv", header = F, sep = ",")
for(u in 2027:2030){
  scenarioConsumption <- scenario2CombSeriesOfImpactEnergy[[paste0("y", u)]][["consumption"]]
  scenario2CombSeriesOfImpactEnergy[[paste0("y", u)]][["consumption"]][4:ncol(scenarioConsumption)] <-  scenarioConsumption[4:ncol(scenarioConsumption)] * scenario1EnergySE2
}
...
scenarioTable<-read.table("D:/My_Development/RProjects/lcd-scenario/_AN/limbah_pertanian_energi_scen.csv", header =  T, sep=",")

###END: energy-waste-fertilizer ####




###BEGIN: Scenario energy-fertilizer simulation####
scenario3CombFD <- read.table("D:/My_Development/RProjects/lcd-scenario/_AN/energi_pertanian_fd.csv", header = T, sep = ",")
ef_fert <- read.table("D:/My_Development/RProjects/lcd-scenario/_AN/EF_Scen2.csv", header = T, sep = ",")
scenario3CombSeriesOfOutput <- data.frame(Sektor=ioSector[,1], Kategori=ioSector[,2])
scenario3CombSeriesOfGDP <- data.frame(Sektor=ioSector[,1], Kategori=ioSector[,2])
proportionGDP <- bauSeriesOfGDP$y2015 / ioTotalOutput

# 1. calculate new output and satellite based on intervention on final demand
scenario3CombSeriesOfImpactEnergy <- list()
scenario3CombSeriesOfImpactFertilizer <- list()
for(i in 0:iteration){
  t_curr <- initialYear + i
  eval(parse(text=paste0("scenarioFD <- scenario3CombFD$y", t_curr))) 
  
  scenarioOutput <- ioLeontiefInverse %*% scenarioFD
  scenario3CombSeriesOfOutput <- cbind(scenario3CombSeriesOfOutput, scenarioOutput)

  scenarioGDP <- scenarioOutput * proportionGDP
  scenario3CombSeriesOfGDP <- cbind(scenario3CombSeriesOfGDP, scenarioGDP)
  
  eval(parse(text= paste0("scenario3CombSeriesOfImpactEnergy$y", t_curr, " <- functionSatelliteImpact('energy', satellite = satelliteEnergy, matrix_output = as.matrix(scenarioOutput), emission_factor = emissionFactorEnergy)")))
  eval(parse(text= paste0("scenario3CombSeriesOfImpactFertilizer$y", t_curr, " <- functionSatelliteImpact('agriculture', satellite = satelliteAgriculture, matrix_output = as.matrix(scenarioOutput), emission_factor = ef_fert)")))
}
colnames(scenario3CombSeriesOfGDP)[3:17] <- as.character(listYear[2:16])
colnames(scenario3CombSeriesOfGDP)[3:17] <- as.character(listYear[2:16])

# 2. next do intervention on satellite
scenario1EnergySE1 <- read.table("D:/My_Development/RProjects/lcd-scenario/_DB/18_persentase_bahan_bakar_sken1_tahap1.csv", header = F, sep = ",")
for(t in 2017:2026){
  scenarioConsumption <- scenario3CombSeriesOfImpactEnergy[[paste0("y", t)]][["consumption"]]
  scenario3CombSeriesOfImpactEnergy[[paste0("y", t)]][["consumption"]][4:ncol(scenarioConsumption)] <-  scenarioConsumption[4:ncol(scenarioConsumption)] * scenario1EnergySE1
}

scenario1EnergySE2 <- read.table("D:/My_Development/RProjects/lcd-scenario/_DB/18_persentase_bahan_bakar_sken1_tahap2.csv", header = F, sep = ",")
for(u in 2027:2030){
  scenarioConsumption <- scenario3CombSeriesOfImpactEnergy[[paste0("y", u)]][["consumption"]]
  scenario3CombSeriesOfImpactEnergy[[paste0("y", u)]][["consumption"]][4:ncol(scenarioConsumption)] <-  scenarioConsumption[4:ncol(scenarioConsumption)] * scenario1EnergySE2
}

inter_fert <- read.table("D:/My_Development/RProjects/lcd-scenario/_AN/fert_scen.csv", header = T, sep = ",")
for(u in 2016:2030){
  scenarioConsumption <- scenario3CombSeriesOfImpactFertilizer[[paste0("y", u)]][["consumption"]]
  scenario3CombSeriesOfImpactFertilizer[[paste0("y", u)]][["consumption"]][4:ncol(scenarioConsumption)] <-  scenarioConsumption[4:ncol(scenarioConsumption)] * inter_fert
}

# Energy cons
resultCombFertConsumption <- scenario3CombSeriesOfImpactFertilizer[[1]][[1]]
resultCombFertConsumption$year <- initialYear
resultCombFertConsumption <- resultCombFertConsumption[, c("year", names(scenario3CombSeriesOfImpactFertilizer[[1]][[1]]))]
for(t in 1:iteration){
  t_curr <- initialYear + t
  add.row <- data.frame(scenario3CombSeriesOfImpactFertilizer[[t+1]][[1]]) # [[2]] for emission
  add.row$year <- t_curr
  add.row <- add.row[, names(resultCombFertConsumption)]
  resultCombFertConsumption <- data.frame(rbind(resultCombFertConsumption, add.row), stringsAsFactors = FALSE)
}
...
# Energy emission 
resultCombFertEmission <- scenario3CombSeriesOfImpactFertilizer[[1]][[2]]
resultCombFertEmission$year <- initialYear
resultCombFertEmission <- resultCombFertEmission[, c("year", names(scenario3CombSeriesOfImpactFertilizer[[1]][[2]]))]
for(t in 1:iteration){
  t_curr <- initialYear + t
  add.row <- data.frame(scenario3CombSeriesOfImpactFertilizer[[t+1]][[2]]) # [[2]] for emission
  add.row$year <- t_curr
  add.row <- add.row[, names(resultCombFertEmission)]
  resultCombFertEmission <- data.frame(rbind(resultCombFertEmission, add.row), stringsAsFactors = FALSE)
}
...
# Energy cons
resultCombEnergyConsumption <- scenario3CombSeriesOfImpactEnergy[[1]][[1]]
resultCombEnergyConsumption$year <- initialYear
resultCombEnergyConsumption <- resultCombEnergyConsumption[, c("year", names(scenario3CombSeriesOfImpactEnergy[[1]][[1]]))]
for(t in 1:iteration){
  t_curr <- initialYear + t
  add.row <- data.frame(scenario3CombSeriesOfImpactEnergy[[t+1]][[1]]) # [[2]] for emission
  add.row$year <- t_curr
  add.row <- add.row[, names(resultCombEnergyConsumption)]
  resultCombEnergyConsumption <- data.frame(rbind(resultCombEnergyConsumption, add.row), stringsAsFactors = FALSE)
}
...
# Energy emission 
resultCombEnergyEmission <- scenario3CombSeriesOfImpactEnergy[[1]][[2]]
resultCombEnergyEmission$year <- initialYear
resultCombEnergyEmission <- resultCombEnergyEmission[, c("year", names(scenario3CombSeriesOfImpactEnergy[[1]][[2]]))]
for(t in 1:iteration){
  t_curr <- initialYear + t
  add.row <- data.frame(scenario3CombSeriesOfImpactEnergy[[t+1]][[2]]) # [[2]] for emission
  add.row$year <- t_curr
  add.row <- add.row[, names(resultCombEnergyEmission)]
  resultCombEnergyEmission <- data.frame(rbind(resultCombEnergyEmission, add.row), stringsAsFactors = FALSE)
}
...
write.table(scenarioCombSeriesOfEmissionBySector, "pertanian_energi_emisi.csv", row.names = F, sep=",")
write.table(scenario2CombSeriesOfGDP, "pertanian_energi_gdp.csv", row.names = F, sep=",")
write.table(scenarioCombResult, "pertanian_energi_scen.csv", row.names = F, sep=",")

###END: energy-fertilizer ####
